...
public class CancelJobWindow extends Window {
	...
	public CancelJobWindow(ServiceInfoModel server, HomepageTab tabPanel, JobStatusModel model){
		this.setWidth(400);
		this.setResizable(false);
		this.setHeading(UIContext.Constants.cancelJob());
		...
		LabelField label = new LabelField(UIContext.Constants.cancelJobDescription());
		this.add(label,td);
		
		TableData labelTD = new TableData();
		labelTD.setWidth("26%");
		label = new LabelField(UIContext.Constants.cancelFor()+ UIContext.Constants.delimiter());
		...
		okButton.addSelectionListener(new SelectionListener<ButtonEvent>(){
				...
				public void componentSelected(ButtonEvent ce) {					
					...
					service.cancelJob(webServer, jobModel,isRunSameJob(), new BaseAsyncCallback<Integer>(){
						...
						public void onSuccess(Integer result) {
							if(result==0){
								...
								Info.display(UIContext.Constants.productName(), UIContext.Constants.cancelJobSubmitSuccessfully());
							}else{
								Utils.showMessage(UIContext.Constants.productName(),MessageBox.ERROR,UIContext.Constants.failToCancelJob());
							}
							processAfter(true);
						}
					});
		...
		helpButton.addSelectionListener(new SelectionListener<ButtonEvent>(){
			...
			@Override
			public void componentSelected(ButtonEvent ce) {
				Utils.showURL(UIContext.helpLink+HelpLinkItem.CANCEL_WINDOW);
			}
