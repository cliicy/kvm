...
public class DatePicker extends BoxComponent {
	...
	protected void onRender(Element target, int index) {
		...
		for (int i = 0; i < 7; i++) {
			days.getCellFormatter().getElement(0, i)
					.setAttribute("role", "columnheader");
			days.getCellFormatter().getElement(0, i)
					.setAttribute("aria-label", longdn[i]);
		}
		...
		for (int row = 0; row < 6; row++) {
			if (GXT.isAriaEnabled()) {
				grid.getRowFormatter().getElement(row)
						.setAttribute("role", "row");
			}
			for (int col = 0; col < 7; col++) {
				grid.setHTML(row, col, s);
				if (GXT.isAriaEnabled()) {
					grid.getCellFormatter().getElement(row, col)
	...
	private void createMonthPicker() {
		...
		mpMonths.each(new CompositeFunction() {
			...
			public void doFunction(Element elem, CompositeElement ce, int index) {
				index += 1;
				if (index % 2 == 0) {
					elem.setPropertyInt("xmonth",
							(int) (5 + (Math.round(index * .5))));
				} else {
					elem.setPropertyInt("xmonth",
							(int) (Math.round((index - 1) * .5)));
				}
			}
	...
	private int getCalculatedStartDay() {
		return startDay != Integer.MIN_VALUE ? startDay : Integer
	...
	private void selectToday() {
		if (serverDate == null) {
			...
			setValue(new DateWrapper(serverDate).asDate());
		}
	}
