...
public class DateField extends TriggerField<Date> {
	...
	public class DateFieldMessages extends TextFieldMessages {
		 ...
		 * minValue (defaults to 'The date in this field must be after
		 * {@link #setMinValue} ').
		 * 
		 * @param minText
		 *            the min text
	...
	protected boolean validateValue(String value) {
		...
		if (date == null) {
			...
			if (getMessages().getInvalidText() != null) {
				error = Format.substitute(getMessages().getInvalidText(),
						value, format.getPattern().toUpperCase());
			} else {
				error = GXT.MESSAGES.dateField_invalidText(value, format
