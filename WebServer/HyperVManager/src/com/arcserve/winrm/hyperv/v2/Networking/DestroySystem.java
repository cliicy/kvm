...
public abstract class DestroySystem implements Method {
     ...
     * @throws Exception 
     * 
     * @see http://msdn.microsoft.com/en-us/library/hh850044(v=vs.85).aspx
     * */
    public static Output invoke(Classes classes, Client conn, String switchUuid) throws Exception {
        ...
        ClassReference targetSwitch = classes.createReference("Msvm_VirtualEthernetSwitch");
        targetSwitch.addSelector("Name", switchUuid);
        
        ClassReference job = null;
        Integer retVal = -1;
        
        for (String resPart : vsms.invokeMethod(conn, "DestroySystem", classes.ARG("AffectedSystem", targetSwitch))) {
