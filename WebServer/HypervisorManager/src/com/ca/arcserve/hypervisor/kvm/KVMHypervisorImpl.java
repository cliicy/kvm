package com.ca.arcserve.hypervisor.kvm;
...
public class KVMHypervisorImpl implements IHypervisor {
	private CAVirtualInfrastructureManager kvmManager = null;
	private Boolean isConnected() {
		if(kvmManager != null) return true;
		return false;
	}
	private static final Logger logger = Logger.getLogger(KVMHypervisorImpl.class);
	@Override
	public Boolean connect(HypervisorInfo hypervisor) throws HypervisorException{
		try {
			kvmManager = CAVMwareInfrastructureManagerFactory.getCAVMwareVirtualInfrastructureManager(hypervisor.getName(), hypervisor.getUsername(), hypervisor.getPassword(), hypervisor.getProtocol(), true, hypervisor.getPort());
		} catch(InvalidLoginException e){
			logger.error("Failed to create connection to kvm(" + hypervisor.getName() + ") invalid credential");
			throw new HypervisorException(HypervisorErrorCode.ERROR_INVALID_CREDENTIAL,"Invalid credential");
		} catch (Exception e) {
			logger.error("Failed to create connection to kvm(" + hypervisor.getName() + ")");
	...
	public List<SubHypervisorInfo> getSubHosts() {
		if(isConnected()){
			try {
				...
				for(ESXNode n: kvmManager.getESXNodeList()) {
	...
	public String createVM(SubHypervisorInfo subHost, VMCreationConfiguration config) throws HypervisorException{
		if(isConnected()){
			try {
				...
				CAVMDetails detail = kvmManager.createVMwareVM(
						config.getName(),
	...
	public boolean deleteVM(String uuid) throws HypervisorException{
		if(isConnected())
			try {
				return kvmManager.deleteVM(null, uuid);
			} catch (Exception e) {
	...
	public boolean startVM(String uuid) throws HypervisorException{
		if(isConnected())
			try {
				return kvmManager.powerOnVM(null, uuid);
			} catch (Exception e) {
	...
	public boolean stopVM(String uuid) throws HypervisorException{
		if(isConnected())
			try {
				kvmManager.powerOffVM(null, uuid,true);
				return true;
			} catch (Exception e) {
	...
	public String createHypervisorNetwork(String subHost, String networkName) throws HypervisorException{
		if(isConnected())
			try {
				VMNetworkConfigInfo network = kvmManager.createHostNetwork(subHost, networkName);
	...
	public boolean removeHypervisorNetwork(String subHost, String networkName) throws HypervisorException{
		if(isConnected())
			try {
				return kvmManager.removeHostNetwork(subHost, networkName);
			} catch (Exception e) {
				logger.error("Failed to create network");
	...
	public VirtualMachineInfo getVirtualMachineInfo(String uuid) throws HypervisorException{
		if(isConnected()){
			try{
				VM_Info vmInfo =  kvmManager.getVMInfo(null, uuid);
	...
	public boolean changeCDRom(String uuid, CDRomInfo cdInfo) throws HypervisorException{
		if(isConnected()){
			try{
				...
				kvmManager.reconfigVMCDRom(null, uuid, cdRom);
				return true;
			} catch (Exception e){
	...
	public String addNicCard(String vmuuid,String subHost,NetworkInfo networkInfo) throws HypervisorException{
		if(isConnected()){
			try{
				...
				kvmManager.addNicCard(null, vmuuid, subHost, nicCard);
				return mac;
			} catch (Exception e){
	...
	public List<VirtualMachineInfo> getVMList(SubHypervisorInfo subHost) {
		if(isConnected()){
			try {
				List<VM_Info> vmList = kvmManager.getVMNames(getESXNode(subHost), true);
	...
	public String createSnapshot(String vmuuid, String snapshotName,
		...
		if(isConnected()){
			try{
				return kvmManager.createSnapshot(null, vmuuid, snapshotName, description);
			}catch(Exception e){
	...
	public int enableChangeTracking(String vmuuid, boolean isEnable) {
		if(isConnected()){
			try{
				if(isEnable){
					return kvmManager.enableChangeTracking(null, vmuuid);
				}else{
					return kvmManager.disableChangeTracking(null, vmuuid) == true ? 0 : -1;
				}
			}catch(Exception e){
				logger.error("create snapshot failed",e);
			}
	...
	public boolean removeSnapshot(String vmuuid, String snapshotId) {
		if(isConnected()){
			try{
				if(snapshotId == null){
					return kvmManager.removeAllSnapshots(null, vmuuid); 
				}
				return kvmManager.removeSnapshot(null, vmuuid, snapshotId);
			}catch(Exception e){
	...
	public List<SnapshotDiskInfo> getSnapshotDiskInfo(String vmuuid,
		...
		if(isConnected()){
			try{
				List<Disk_Info> diskList = kvmManager.getsnapDiskInfo(null, vmuuid, snapshotId);
				if(diskList != null){
	...
	public SnapshotDiskSectorInfo generateBitMapForDisk(String vmuuid,
		...
		if(isConnected()){
			try{
				Disk_Sector_Info diskSectorInfo = kvmManager.getUsedDiskBlocks(null, vmuuid, snapshotId, diskChangeId, diskDeviceKey, filePath, chunkSize, sectorSize);
	...
	public List<ResourcePool> getResourcePoolList(SubHypervisorInfo subHost) {
		if(isConnected()){
			try{
				List<ResPool_Info> resPoolList = kvmManager.getresPool(getESXNode(subHost));
				if(resPoolList != null){
	...
	private String getDcPath(SubHypervisorInfo subHypervisorInfo){
		...
		try {
			VMwareServerType serverType = kvmManager.getVMwareServerType();
			if(serverType == VMwareServerType.virtualCenter){
				dcPath = kvmManager.getDcPath(getESXNode(subHypervisorInfo));
			}
		} catch (Exception e) {
	...
	public List<StorageInfo> getStorageList(SubHypervisorInfo subHost) {
		if(isConnected()){
			try {
				List<ESXDatastore> dsList = kvmManager.getESXHostDataStoreListWithDetail(getESXNode(subHost));
	...
	public VirtualMachineDefaultSetting getVirtualMachineDefaultSetting(String subHost,
		...
		if(isConnected()){
			...
			try {
				CAVirtualMachineDefaultSetting setting = kvmManager.getVMDefaultSetting(getESXNode(subHypervisorInfo),guestOS);
	...
	public boolean removeNicCard(String vmuuid, String subHost,
		...
		if(isConnected()){
			try {
				kvmManager.removeNicCardByNetworkName(null, vmuuid, subHost, networkName);
				return true;
			} catch (Exception e) {
				logger.error("Failed to getVirtualMachineDefaultSetting",e);
			}
	...
	public List<String> getNetworkList(SubHypervisorInfo subHost) {
		if(isConnected()){
			try {
				...
				ArrayList<VirtualNetworkInfo> networkInfos = kvmManager.getVirtualNetworkList(getESXNode(subHost));
	...
	public boolean addSerialPort(String vmuuid, String subHost,
		...
		if(isConnected()){
			try{
				return kvmManager.addSerialPort(null, vmuuid, subHost, datastore,fileName);
			} catch (Exception e){
	...
	public boolean addConfigurationParameter(String vmuuid, String subHost,
		...
		if(isConnected()){
			try{
				return kvmManager.addConfigurationParameter(null, vmuuid, subHost, parameters);
			} catch (Exception e){
	...
	public boolean changeBootOrder(String vmuuid, BootDeviceType bootDeviceType)
		...
		if(isConnected()){
			try{
				...
				return kvmManager.reconfigBootOrder(null, vmuuid, bootDevice);
			} catch (Exception e){
	...
	public String createNasDatastore(SubHypervisorInfo subHost,
		...
		if(isConnected()){
			try{
				...
				String result = kvmManager.createNasDatastore(getESXNode(subHost), ds);
				return result;
			}catch(Exception e){
	...
	public boolean removeDatastore(SubHypervisorInfo subHost,
		...
		if(isConnected()){
			try {
				return kvmManager.removeDatastore(getESXNode(subHost), datastoreName);
			} catch (Exception e) {
	...
	public void refreshDatastore(SubHypervisorInfo subHost,
		...
		if(isConnected()){
			try {
				kvmManager.refreshDatastore(getESXNode(subHost), datastoreName);
			} catch (Exception e) {
	...
	public PowerState getVMPowerStatus(String vmuuid) throws HypervisorException{
		...
		if(isConnected()){
			try {
				powerState state = kvmManager.getVMPowerstate(null, vmuuid);
	...
	public boolean uploadVMNvRamFile(String vmuuid, String file) {
		if(isConnected()){
			try{
				return kvmManager.setVMNVRAMFile(null, vmuuid, file);
			} catch (Exception e) {
	...
	public boolean shutdownOS(String uuid) throws HypervisorException {
		if(isConnected())
			try {
				kvmManager.shutdownVM(null, uuid);
				return true;
			} catch (Exception e) {
				logger.error("Failed to shutdown kvm vm", e);
